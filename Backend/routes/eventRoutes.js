...
router.post("/", protect, authorize("admin"), upload.single("eventImage"), createEvent);          // admin specific route
router.get("/", getEvents);                      // Get all events
router.get("/stats", protect, authorize("admin"), getEventStats);             // Get event stats

router.get("/:id", protect, getEventById);                // Get one
router.put("/:id", protect, authorize("admin"), upload.single("eventImage"), protect, updateEvent);        // admin specific route
